...
import (
	...
	"github.com/pai0id/CgCourseProject/internal/mapping"
)
...
// DroidSansMono.ttf
func main() {
	f, err := fontparser.GetFontMap("fonts/DroidSansMono.ttf", 16, 25, 20, 72)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
	}
	...
	shapeMap := mapping.GetShapeMap(mapping.NewContext(16, 25, 8, 12, 12.0), mapping.FontMapToCellSlice(f))
	fmt.Println(len(shapeMap))
	...
	// 		fmt.Println()
	// 	}
	// 	fmt.Println(brightnes.GetBrightness(brightnes.Cell(charMat[cc1])))
	...
	// 		fmt.Println()
	// 	}
	// 	fmt.Println(brightnes.GetBrightness(brightnes.Cell(charMat[cc2])))
